...
exports.getCourse = asyncHandler(async (req, res, next) => {
  ...
  if (!course) {
    return next(
      new ErrorResponse(`No course with the is of ${req.params.id}`), 404
    );
  }
...
exports.addCourse = asyncHandler(async (req, res, next) => {
  ...
  // Make sure user is owner of bootcamp to add an course to it
...
exports.updateCourse = asyncHandler(async (req, res, next) => {
  ...
  if (!course) {
    return next(
      new ErrorResponse(`No course found with the id of ${req.params.bootcampId}`), 404
    );
  }
